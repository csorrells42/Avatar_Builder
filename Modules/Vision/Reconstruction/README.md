...
This module owns subject-gated 3DDFA observation persistence, avatar model construction, history audits, and local review reports. MediaPipe and OpenCV remain outside this module, while the active 3DDFA sidecar client lives under `Vision.Onnx`.
...
The active Avatar System dashboard is a lightweight live report: subject gate, capture state, capture quality, fast MediaPipe/OpenCV cue status, 3DDFA_V2 ONNX reconstruction status, current face-frame geometry, model confidence/coverage, Avatar Model Progress link, and 3DDFA Last 5 link.
